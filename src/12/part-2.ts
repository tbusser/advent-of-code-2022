...
function neighborPredicate(
	...
	// We're going from high to low. In order for the neighbor to be a valid
	// next step it should not be more than 1 lower then the current height.
...
function solve(grid: Grid, start: Coordinate): number {
	...
	while (priorityQueue.length > 0) {
		...
		// When the current item is a valid start position, return how many
		// steps it took to get there.
		if (grid.isStartCoordinate(coordinate)) {
			return depth;
		}
...
async function findSolution(input: string): Promise<number> {
	...
	// Work back from the end position to a valid start position with the
	// shortest possible path.
	return solve(grid, grid.endPosition);
}
